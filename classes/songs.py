...
class Songs:
    ...
    def encode_categories(self):
        # Encode the categories to turn text into categorical number
        category_encoder = LabelBinarizer().fit(self.categories)

        for song in self.song_objects:
            # Use the fitted encoder to transform the category
            num_rows = song.mfcc_features.shape[0]
            category_column = np.full((num_rows, 1), song.category)
            category_encoded = category_encoder.transform(category_column)
